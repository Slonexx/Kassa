<div class="row gradient rounded p-2 pb-2" style="margin-top: -1rem">
    <div class="col-9" style="margin-top: 0.5rem"> <span id="HEAD_TOP_SERVICE" class="text-white" style="font-size: 20px">  </span> </div>
    <div class="col-3 text-center">
        <img src="https://dev.smartrekassa.kz/logoHeadWhite.png" width="80%"  alt="">
    </div>
</div>
    <script>
        function NAME_HEADER_TOP_SERVICE(name){
            window.document.getElementById('HEAD_TOP_SERVICE').innerText = name
        }
    </script>
{{-- Настройки &#8594; настройки интеграции --}}
